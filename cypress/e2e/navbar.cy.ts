describe("Navbar", () => {
	it("simulates clicks on navbar links", () => {
		...
		cy.get('[role="banner"]').contains("a", "Sobre Nós").click();
		cy.url().should("match", /\/about$/);

		cy.get('[role="banner"]').contains("a", "Início").click();
		cy.url().should("match", /\/dashboard$/);

		cy.get('[role="banner"]').contains("a", "DoceGestão").click();
		cy.url().should("match", /\/dashboard$/);
	});
	...
	it("simulates clicks on navbar action controls", () => {
		...
		cy.get('[role="banner"]').contains("a", "Perfil").should("be.visible");
		...
		cy.get('[role="banner"]').contains("a", "Perfil").click();
